...
help:
	...
	@echo "  make pi-run-esp      - Run Pi gateway with ESP32 serial ESC control"
	@echo "  make pi-run-esc      - Run legacy direct Pi GPIO ESC control"
	...
	@echo "  make pi-connect-esp  PC_IP=<ip> - Connect Pi ESP32 ESC mode to the PC backend with LiDAR streaming"
	@echo "  make pi-connect-esc  PC_IP=<ip> - Connect legacy direct Pi GPIO ESC mode to the PC backend"
...
pi-connect-esp:
	@if [ -z "$(HOST_IP)" ]; then \
		echo "Usage: make pi-connect-esp PC_IP=192.168.1.25"; \
		exit 1; \
	fi
	@if [ ! -f "$(PI_VENV_DIR)/bin/activate" ]; then \
		echo "Missing $(PI_VENV_DIR)/bin/activate. Run: python3 -m venv $(PI_VENV_DIR) && make pi-setup"; \
	...
	@. "$(PI_VENV_DIR)/bin/activate"; \
	$(PI_GATEWAY_ENV) PI_MOTOR_DRIVER=esp python3 pi/gateway.py
...
pi-connect-webrtc-esp:
	@if [ -z "$(HOST_IP)" ]; then \
		echo "Usage: make pi-connect-webrtc-esp PC_IP=192.168.1.25"; \
		exit 1; \
	fi
	@if [ ! -f "$(PI_VENV_DIR)/bin/activate" ]; then \
		echo "Missing $(PI_VENV_DIR)/bin/activate. Run: python3 -m venv $(PI_VENV_DIR) && make pi-setup"; \
	...
	@. "$(PI_VENV_DIR)/bin/activate"; \
	trap 'kill 0 >/dev/null 2>&1' INT TERM EXIT; \
	$(PI_GATEWAY_ENV) PI_MOTOR_DRIVER=esp PI_CAMERA_JPEG_ENABLED=0 python3 pi/gateway.py & \
	$(PI_WEBRTC_ENV) python3 pi/webrtc_publisher.py & \
	wait
